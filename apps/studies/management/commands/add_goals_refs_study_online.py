"""
Add the Goals as Reference Points (Decision Making Vignettes) study to the online database.
Reads study_config.json from apps/studies/assets/irb/goals-refs/.
...
DEFAULT_CONSENT = (
    "You will be asked to complete 3–6 short vignette scenarios about decision-making and judgments. "
...
class Command(BaseCommand):
    help = "Add Goals as Reference Points (Decision Making Vignettes) study to online database (reads study_config.json)"

    def handle(self, *args, **options):
        base = Path(__file__).resolve().parent.parent.parent.parent.parent
        config_dir = base / "apps" / "studies" / "assets" / "irb" / "goals-refs"
        config_path = config_dir / "study_config.json"

        if not config_path.exists():
            self.stdout.write(self.style.ERROR(
                "Goals-refs study config not found.\n"
            ))
            self.stdout.write(
                f"Create: {config_path}\n"
                "Copy from: apps/studies/assets/irb/hr-sjt/study_config.json\n"
        ...
        slug = cfg.get("slug") or "goals-refs"
        title = cfg.get("title") or "Goals as Reference Points and Risk Taking"
        ...
        credit_value = float(cfg.get("credit_value", 0.5))
        mode = cfg.get("mode") or "online"
        consent_text = cfg.get("consent_text") or DEFAULT_CONSENT
        irb_status = cfg.get("irb_status") or "pending"
        # Use config URL when set; leave blank until frontend is deployed or Qualtrics link is ready
        external_link = (cfg.get("external_link") or "").strip()
        ...
        if not created:
            ...
            study.external_link = external_link
        ...
        self.stdout.write(self.style.SUCCESS(f"  External link: {study.external_link or '(not set)'}"))
        self.stdout.write(
            f"  HSIRB view: /studies/protocol/{study.id}/ (or protocol submission list)"
        )
