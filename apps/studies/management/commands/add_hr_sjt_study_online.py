"""
Add the HR Situational Judgment Test study to the online database.
Reads study_config.json from apps/studies/assets/irb/hr-sjt/.
...
DEFAULT_CONSENT = (
    "You will be asked to complete the HR Situational Judgment Test. "
...
class Command(BaseCommand):
    help = "Add HR SJT study to online database (reads study_config.json)"

    def handle(self, *args, **options):
        base = Path(__file__).resolve().parent.parent.parent.parent.parent
        config_dir = base / "apps" / "studies" / "assets" / "irb" / "hr-sjt"
        config_path = config_dir / "study_config.json"

        if not config_path.exists():
            self.stdout.write(self.style.ERROR(
                "HR SJT study config not found.\n"
            ))
            self.stdout.write(
                f"Create: {config_path}\n"
                "Copy from: apps/studies/assets/irb/whole-person-fit/study_config.json\n"
        ...
        slug = cfg.get("slug") or "hr-sjt"
        title = cfg.get("title") or "HR Situational Judgment Test"
