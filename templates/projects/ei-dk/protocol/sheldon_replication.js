...
class SheldonReplicationStudy {
    ...
    async loadTestItems() {
        // Load 36-item EI assessment (3 videos × 12 items each)
        ...
        try {
            const response = await fetch('/static/projects/ei-dk/data/ei36_item_bank.json', { cache: 'no-store' });
            if (!response.ok) {
            ...
            // Validate
            if (!Array.isArray(rawItems) || rawItems.length !== 36) {
                throw new Error(`Expected 36 items, got ${rawItems.length}`);
            }
        ...
        const videoHtml = item.type === 'video' ? `
            <div class="mb-6">
                ...
                <video 
                    ...
                    <source src="/static/projects/ei-dk/videos/video1.mp4" type="video/mp4">
